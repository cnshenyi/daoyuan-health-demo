<template>
  <div class="patient-detail">
    ...
    <!-- ========== 长期计划面板 ========== -->
    <div v-if="showLongTermPanel" class="panel-overlay" @click.self="showLongTermPanel = false">
      <div class="panel">
        <div class="panel-header"><h4>长期管理计划</h4><span class="panel-close" @click="showLongTermPanel = false">&times;</span></div>
        <div class="panel-body">
          <label class="field-label">管理目标</label>
          <div class="check-options">
            <span v-for="g in longTermGoals" :key="g" :class="['check-opt', longTermForm.goals.includes(g) && 'active']" @click="toggleLongTermGoal(g)">{{ g }}</span>
          </div>

          <label class="field-label">阶段计划</label>
          <div v-for="(phase, idx) in longTermForm.phases" :key="idx" class="phase-card">
            <div class="phase-header">
              <span class="phase-num">阶段 {{ idx + 1 }}</span>
              <span v-if="longTermForm.phases.length > 1" class="med-remove" @click="longTermForm.phases.splice(idx, 1)">&times;</span>
            </div>
            <input v-model="phase.name" class="form-input full" placeholder="阶段名称（如：血糖强化控制期）" />
            <div class="med-form-row" style="margin-top:6px">
              <input v-model="phase.duration" class="form-input flex1" placeholder="周期" />
              <input v-model="phase.target" class="form-input flex2" placeholder="目标指标" />
            </div>
            <textarea v-model="phase.measures" class="form-textarea" rows="2" placeholder="具体措施：药物调整、饮食方案、运动计划等" style="margin-top:6px"></textarea>
          </div>
          <div class="add-row" @click="addPhase">+ 添加阶段</div>

          <label class="field-label">随访安排</label>
          <div class="check-options">
            <span v-for="f in followUpOptions" :key="f" :class="['check-opt', longTermForm.followUpFreq === f && 'active']" @click="longTermForm.followUpFreq = f">{{ f }}</span>
          </div>

          <label class="field-label">注意事项</label>
          <textarea v-model="longTermForm.notes" class="form-textarea" rows="2" placeholder="长期管理注意事项、患者教育要点等"></textarea>

          <div class="panel-actions">
            <button class="panel-btn secondary" @click="showLongTermPanel = false">取消</button>
            <button class="panel-btn primary" @click="submitLongTerm">保存计划</button>
          </div>
        </div>
      </div>
    </div>
...
const showLongTermPanel = ref(false)
...
// ===== 基础信息 =====
const patientBasicInfoMap: Record<string, { allergies: string[], pastHistory: string[], familyHistory: string[], smoking: string, drinking: string, occupation: string, vaccinations: string[] }> = {
  '1': { allergies: ['青霉素', '海鲜'], pastHistory: ['2型糖尿病', '高血压', '高血脂'], familyHistory: ['父亲：高血压', '母亲：糖尿病'], smoking: '已戒烟5年', drinking: '偶尔饮酒', occupation: '企业高管', vaccinations: ['新冠疫苗(3针)', '流感疫苗', '肺炎疫苗'] },
  '3': { allergies: ['磺胺类药物'], pastHistory: ['冠心病', '痛风'], familyHistory: ['父亲：冠心病'], smoking: '吸烟30年', drinking: '经常饮酒', occupation: '退休干部', vaccinations: ['新冠疫苗(3针)', '流感疫苗'] },
  '9': { allergies: [], pastHistory: ['心房颤动', '骨质疏松'], familyHistory: ['母亲：高血压'], smoking: '不吸烟', drinking: '不饮酒', occupation: '退休教师', vaccinations: ['新冠疫苗(3针)', '流感疫苗', '带状疱疹疫苗'] }
}
const defaultBasicInfo = { allergies: [], pastHistory: [], familyHistory: [], smoking: '未知', drinking: '未知', occupation: '未知', vaccinations: [] }
const currentBasicInfo = computed(() => patientBasicInfoMap[pid] || defaultBasicInfo)
const patientBasicInfo = computed(() => {
  const info = currentBasicInfo.value
  return [
    { label: '过敏史', value: info.allergies.length ? info.allergies.join('、') : '无' },
    { label: '既往史', value: info.pastHistory.length ? info.pastHistory.join('、') : '无' },
    { label: '家族史', value: info.familyHistory.length ? info.familyHistory.join('、') : '无' },
    { label: '吸烟', value: info.smoking },
    { label: '饮酒', value: info.drinking },
    { label: '职业', value: info.occupation },
    { label: '疫苗接种', value: info.vaccinations.length ? info.vaccinations.join('、') : '无' }
  ]
})

// ===== 危险因素 =====
const patientRiskMap: Record<string, { name: string, desc: string, level: string, levelText: string }[]> = {
  '1': [
    { name: '高糖饮食', desc: '患者偏好甜食，碳水化合物摄入偏高', level: 'high', levelText: '高风险' },
    { name: '久坐少动', desc: '工作性质导致每日久坐超过8小时', level: 'medium', levelText: '中风险' },
    { name: '睡眠不足', desc: '经常加班，平均睡眠不足6小时', level: 'medium', levelText: '中风险' },
    { name: '工作压力', desc: '长期高压工作环境，精神紧张', level: 'low', levelText: '低风险' }
  ],
  '3': [
    { name: '长期吸烟', desc: '吸烟30年，每日1包以上', level: 'high', levelText: '高风险' },
    { name: '经常饮酒', desc: '每周饮酒3-4次，量较大', level: 'high', levelText: '高风险' },
    { name: '高嘌呤饮食', desc: '偏好海鲜、内脏等高嘌呤食物', level: 'medium', levelText: '中风险' }
  ],
  '9': [
    { name: '跌倒风险', desc: '骨质疏松合并步态不稳', level: 'high', levelText: '高风险' },
    { name: '出血风险', desc: '长期服用华法林抗凝', level: 'medium', levelText: '中风险' }
  ]
}
const patientRiskFactors = computed(() => patientRiskMap[pid] || [])

// ===== 检查结果分组（同类型近3次并列比较） =====
const examTypeLabel: Record<string, string> = { blood: '血液检验', ecg: '心电图/超声', biochem: '生化检查', imaging: '影像检查', ultrasound: '超声检查', other: '其他检查' }
const examGroups = computed(() => {
  const exams = patientExams.value
  ...
  return Object.entries(grouped).map(([type, list]) => ({
    type,
    exams: list.sort((a, b) => b.date.localeCompare(a.date)).slice(0, 3)
  }))
})

// ===== 长期计划 =====
const longTermGoals = ['血糖达标', '血压控制', '血脂管理', '体重管理', '并发症预防', '生活方式改善', '心理健康', '营养均衡']
const followUpOptions = ['每周一次', '每两周一次', '每月一次', '每季度一次']
const longTermForm = reactive({
  goals: [] as string[],
  phases: [{ name: '', duration: '', target: '', measures: '' }],
  followUpFreq: '每月一次',
  notes: ''
})
const toggleLongTermGoal = (g: string) => {
  const i = longTermForm.goals.indexOf(g)
  i >= 0 ? longTermForm.goals.splice(i, 1) : longTermForm.goals.push(g)
}
const addPhase = () => longTermForm.phases.push({ name: '', duration: '', target: '', measures: '' })
const submitLongTerm = () => { showLongTermPanel.value = false; toast('长期计划已保存') }
</script>
...
.sec-title { font-size: 15px; font-weight: 600; color: #1E3A5F; margin: 0 0 10px; }
...
.action-grid { display: grid; grid-template-columns: repeat(5, 1fr); gap: 8px; padding: 12px 16px; }
...
.action-btn span { font-size: 13px; color: #333; font-weight: 500; }
...
.rx-diag { font-size: 14px; font-weight: 600; color: #1E3A5F; }
.rx-date { font-size: 11px; color: #999; }
.med-row { display: flex; justify-content: space-between; align-items: center; padding: 4px 0; }
.med-name { font-size: 14px; color: #333; font-weight: 500; }
.med-info { font-size: 13px; color: #999; }
...
.exam-items { font-size: 14px; color: #333; margin: 0 0 2px; }
...
.mr-complaint { font-size: 14px; color: #333; margin: 0 0 2px; font-weight: 500; }
.mr-treatment { font-size: 13px; color: #999; margin: 0; }
...
.field-label { font-size: 14px; color: #333; font-weight: 600; display: block; margin: 14px 0 6px; }
.field-label:first-child { margin-top: 0; }

/* 表单控件 */
.form-input { border: 1px solid #e0e0e0; border-radius: 8px; padding: 9px 10px; font-size: 14px; outline: none; background: #f8f9fb; box-sizing: border-box; }
.form-input:focus { border-color: #1E3A5F; background: #fff; }
.form-input.full { width: 100%; }
.form-select { border: 1px solid #e0e0e0; border-radius: 8px; padding: 9px 8px; font-size: 14px; outline: none; background: #f8f9fb; box-sizing: border-box; appearance: auto; }
.form-select:focus { border-color: #1E3A5F; }
.form-textarea { width: 100%; border: 1px solid #e0e0e0; border-radius: 8px; padding: 9px 10px; font-size: 14px; resize: none; outline: none; background: #f8f9fb; box-sizing: border-box; line-height: 1.5; }
...
.check-opt { font-size: 13px; padding: 6px 14px; border-radius: 16px; background: #f0f2f5; color: #666; cursor: pointer; border: 1px solid transparent; transition: all 0.15s; }
...
/* 基础信息网格 */
.basic-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 8px; }
.basic-item { display: flex; flex-direction: column; gap: 3px; padding: 8px 10px; background: #f8f9fb; border-radius: 8px; }
.basic-item:last-child:nth-child(odd) { grid-column: span 2; }
.basic-label { font-size: 11px; color: #999; }
.basic-value { font-size: 14px; color: #333; font-weight: 500; line-height: 1.4; }

/* 危险因素 */
.risk-list { display: flex; flex-direction: column; gap: 8px; }
.risk-item { display: flex; align-items: center; gap: 10px; padding: 10px 12px; background: #f8f9fb; border-radius: 10px; }
.risk-icon { width: 18px; height: 18px; flex-shrink: 0; color: #E67E22; }
.risk-content { flex: 1; display: flex; flex-direction: column; gap: 2px; }
.risk-name { font-size: 14px; font-weight: 600; color: #333; }
.risk-desc { font-size: 12px; color: #999; }
.risk-level { font-size: 11px; padding: 3px 8px; border-radius: 8px; font-weight: 600; white-space: nowrap; }
.risk-level.high { background: rgba(229,57,53,0.1); color: #E53935; }
.risk-level.medium { background: rgba(230,126,34,0.1); color: #E67E22; }
.risk-level.low { background: rgba(46,139,87,0.1); color: #2E8B57; }

/* 检查结果分组比较 */
.exam-group { margin-bottom: 12px; }
.exam-group-title { font-size: 13px; font-weight: 600; color: #1E3A5F; margin-bottom: 8px; padding-left: 2px; }
.exam-compare { display: flex; gap: 8px; overflow-x: auto; }
.exam-compare::-webkit-scrollbar { display: none; }
.exam-col { flex: 1; min-width: 120px; background: #f8f9fb; border-radius: 10px; padding: 10px; display: flex; flex-direction: column; gap: 4px; }
.exam-col:first-child { border-left: 3px solid #1E3A5F; }
.exam-col-date { font-size: 14px; font-weight: 700; color: #1E3A5F; }
.exam-col-items { font-size: 12px; color: #666; margin: 0; line-height: 1.5; }
.exam-col-result { font-size: 12px; color: #2E8B57; margin: 0; font-weight: 500; }
.exam-col-pending { font-size: 12px; color: #C9A962; margin: 0; font-style: italic; }

/* 长期计划面板 */
.phase-card { background: #f8f9fb; border-radius: 10px; padding: 10px; margin-bottom: 8px; position: relative; }
.phase-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 6px; }
.phase-num { font-size: 13px; font-weight: 600; color: #1E3A5F; }
