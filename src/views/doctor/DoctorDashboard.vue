  ...
  <div class="page-container role-dashboard">
    <div class="role-banner doctor-banner">
      <div class="banner-content">
        <h1 class="banner-title">您好，{{ userStore.user?.name }} 医生</h1>
        <p class="banner-subtitle">医生工作台</p>
      </div>
    </div>
    ...
    <div class="stats-row">
      ...
      <div class="stat-card">
        ...
        <span class="stat-label">今日问诊</span>
      </div>
    </div>
      ...
      >
        ...
        <div class="member-info">
          <div class="member-name">
            {{ m.name }}
            <span :class="['status-tag', m.status]">
              {{ statusLabel[m.status] }}
            </span>
          </div>
          <div class="member-meta">
            {{ m.age }}岁 · {{ m.primaryDiagnosis.join('、') }}
          </div>
          <div class="member-meta">末次就诊：{{ m.lastCheckIn }}</div>
        </div>
        <div class="member-score" :class="scoreClass(m.healthScore)">
          {{ m.healthScore }}
        </div>
      </div>
    </div>
...
.member-name { font-size: 15px; font-weight: 600; color: #333; display: flex; align-items: center; gap: 6px; }
...
.member-score { width: 36px; height: 36px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 14px; font-weight: 700; flex-shrink: 0; }
